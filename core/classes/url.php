...
class SiteURL
{
    ...
    public function buildUrl($action = null, $module = null, $append = '', $parameters = null, $ignorePaging = false)
    {
        ...
        if (!$ignorePaging) {
            ...
            if (isset($_GET['sort'])) {
                $parameters['sort'] = SpoonFilter::getGetValue('sort', array('asc', 'desc'), 'asc');
            }
        }
    ...
    private function processQueryString()
    {
        ...
        if (!isset($chunks[0]) || $chunks[0] == '') {
            ...
            $chunks = (array) explode('/', $chunks[0]);
        }

        // nothing provided
        if (empty($chunks)) {
            SpoonHTTP::redirect('/' . SiteLocale::getPreferredLanguage(), 301);
        }
    ...
    {
        // validate language
        if (!in_array($language, SiteLocale::$possibleLanguages)) {
            SpoonHTTP::redirect('/' . SiteLocale::getPreferredLanguage() . '/' . $this->getQueryString());
        }
